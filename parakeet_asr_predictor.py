...
def main():
    ...
    parser.add_argument('--audio_dir', type=str, help='音頻文件目錄路徑')
    parser.add_argument('--output', type=str, help='詳細輸出文件路徑')
    parser.add_argument('--labels', type=str, help='標籤文件路徑（用於計算MER）')
    parser.add_argument('--output_label', type=str, default='task1_answer_Finetuned.txt',
                        help='輸出標籤文件路徑（預設為output_label.txt）')
    parser.add_argument('--json_output', type=str, help='JSON時間戳輸出文件路徑')
    parser.add_argument('--model', type=str, default='nvidia/parakeet-tdt-0.6b-v2', help='模型名稱')
