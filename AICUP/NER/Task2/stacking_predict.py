...
try:
    ...
    print(f"Loading config from {MODEL_CHECKPOINT}...")
    ...
    model = TokenClassificationWithCRF(base_model=BASE_MODEL, config=config)

    # 載入模型權重
    model_weights_path_sf = os.path.join(MODEL_CHECKPOINT, "model.safetensors")
    from safetensors.torch import load_file as load_safetensors_file
    safetensors_dict = load_safetensors_file(model_weights_path_sf)
    print(safetensors_dict)
    print(model)
    model.load_state_dict(safetensors_dict)

    model.to(DEVICE)
    model.eval()
    print(f"Model and tokenizer loaded successfully from {MODEL_CHECKPOINT} on {DEVICE}.")
    print(f"Model backbone is expected to be: {MODEL_CHECKPOINT}")  # 來自儲存的 config.json
