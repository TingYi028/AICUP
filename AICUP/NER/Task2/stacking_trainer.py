...
# 2. 自訂的 Token Classification 模型，整合 CRF 層，使用2個deberta模型stacking
class TokenClassificationWithCRF(PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels

        # 使用兩個 AutoModel 載入預訓練模型
        self.backbone1 = AutoModel.from_pretrained(
            config.name_or_path,
            config=config,
            trust_remote_code=True,
        )

        self.backbone2 = AutoModel.from_pretrained(
            config.name_or_path,
            config=config,
            trust_remote_code=True,
        )

        # 分類層設定 - 使用 Sequential 容器
        classifier_dropout = getattr(config, 'classifier_dropout', None)
        if classifier_dropout is None:
            classifier_dropout = getattr(config, 'hidden_dropout_prob', 0.1)

        # 使用 Sequential 建立分類器，包含 Linear、GELU 和 Dropout
        self.classifier = nn.Sequential(
            nn.Linear(config.hidden_size * 2, config.hidden_size),  # 第一層線性變換
            nn.GELU(),  # GELU 激活函數
            nn.Dropout(classifier_dropout),  # Dropout 層
            nn.Linear(config.hidden_size, config.num_labels)  # 輸出層
        )
    ...
    def forward(
        ...
        sequence_output = torch.cat([sequence_output1, sequence_output2], dim=-1)  # 在最後一維拼接

        # 通過 Sequential 分類器處理拼接後的特徵
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            for name, param in self.model.named_parameters():
                ...
                elif 'backbone1' in name:
                    backbone1_params.append(param)
                elif 'backbone2' in name:
                    backbone2_params.append(param)
                else:
                    other_params.append(param)

            # 創建參數組
            param_groups = [
                {'params': backbone1_params, 'lr': base_lr, 'weight_decay': self.args.weight_decay},
                {'params': backbone2_params, 'lr': base_lr, 'weight_decay': self.args.weight_decay},
            ...
            print(f"Backbone1參數數量: {len(backbone1_params)}")
            print(f"Backbone2參數數量: {len(backbone2_params)}")
...
def main():
    ...
    model1_checkpoint = r"C:\Users\C110151154\PycharmProjects\NeMo\AICUP\NER\ontonotes5\results_ner_ontonotes5_crf\deberta-v3-large\checkpoint-1876\model.safetensors"
    model2_checkpoint = r"C:\Users\C110151154\PycharmProjects\NeMo\AICUP\NER\pii-masking-400k\results_ner_pii_crf\deberta-v3-large\checkpoint-4272\model.safetensors"  # 第二個模型的checkpoint路徑
    train_file_path = r"./datasets_train80_val20/train_bio.json"
    eval_file_path = r"./datasets_train80_val20/val_bio.json"
    output_dir = "./results_ner_microsoft/deberta-v3-large-stacking"
    ...
    # --- 初始化自訂模型 ---
    print(f"初始化自訂Stacking模型: TokenClassificationWithCRF，使用兩個deberta-v3-large模型")
    print(f"模型1 checkpoint: {model1_checkpoint}")
    print(f"模型2 checkpoint: {model2_checkpoint}")
    ...
    try:
        # 載入第一個模型的權重
        print(f"載入第一個模型權重: {model1_checkpoint}")
        if model1_checkpoint.endswith(".safetensors"):
            from safetensors.torch import load_file
            state_dict1 = load_file(model1_checkpoint)
            print(f"成功載入第一個模型 safetensors 權重: {model1_checkpoint}")
        elif model1_checkpoint.endswith(".bin"):
            state_dict1 = torch.load(model1_checkpoint, map_location="cpu")
            print(f"成功載入第一個模型 PyTorch bin 權重: {model1_checkpoint}")
        else:
            raise ValueError("第一個模型不支持的權重檔案格式，請提供 .safetensors 或 .bin 檔案。")

        # 載入第二個模型的權重
        print(f"載入第二個模型權重: {model2_checkpoint}")
        if model2_checkpoint.endswith(".safetensors"):
            from safetensors.torch import load_file
            state_dict2 = load_file(model2_checkpoint)
            print(f"成功載入第二個模型 safetensors 權重: {model2_checkpoint}")
        elif model2_checkpoint.endswith(".bin"):
            state_dict2 = torch.load(model2_checkpoint, map_location="cpu")
            print(f"成功載入第二個模型 PyTorch bin 權重: {model2_checkpoint}")
        else:
            raise ValueError("第二個模型不支持的權重檔案格式，請提供 .safetensors 或 .bin 檔案。")
        ...
        for k, v in state_dict1.items():
            ...
            # 略過 CRF 相關的鍵
            if k.startswith("crf."):
                print(f"跳過第一個模型 CRF 權重鍵: {k}")
                continue
            # 略過 Classifier 相關的鍵（因為 size mismatch）
            if k.startswith("classifier."):
                print(f"跳過第一個模型 Classifier 權重鍵 (避免 size mismatch): {k}")
                continue

            # 為第一個backbone添加前綴
            if k.startswith("backbone."):
                k = "backbone1." + k[len("backbone."):]
        ...
        for k, v in state_dict2.items():
            ...
            # 略過 CRF 相關的鍵
            if k.startswith("crf."):
                print(f"跳過第二個模型 CRF 權重鍵: {k}")
                continue
            # 略過 Classifier 相關的鍵（因為 size mismatch）
            if k.startswith("classifier."):
                print(f"跳過第二個模型 Classifier 權重鍵 (避免 size mismatch): {k}")
                continue

            # 為第二個backbone添加前綴
            if k.startswith("backbone."):
                k = "backbone2." + k[len("backbone."):]

            new_state_dict2[k] = v

        # 合併兩個模型的權重
        final_state_dict = {**new_state_dict1, **new_state_dict2}

        # 載入模型狀態字典
        model.load_state_dict(final_state_dict, strict=False)
        print("兩個模型的 safetensors 權重已成功載入到stacking模型中。")
    except Exception as e:
