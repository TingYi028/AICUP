...
def main():
    # --- 配置 ---
    base_model = "microsoft/mdeberta-v3-base"  # deberta-v3-large 的基礎模型名稱
    ...
    train_file_path = r"./datasets/chinese_bio.json"
    output_dir = "results_ner_microsoft/mdeberta-v3-base-crf"
    ...
    try:
        ...
        # 將 'train' 資料分割為 90% 訓練集和 10% 驗證集
        # 使用固定的 seed 以確保分割的可重複性
        split_datasets = full_dataset['train'].train_test_split(test_size=0.1, seed=42)

        # 重新組合為 DatasetDict，並將 'test' 重新命名為 'validation'
        # 這樣下游的 Trainer API 才能正確識別
        ...
        print(f"成功載入並分割資料集：")
        print(f" - 訓練集: {len(raw_datasets['train'])} 筆")
        print(f" - 驗證集: {len(raw_datasets['validation'])} 筆")

    except Exception as e:
        print(f"載入或分割資料集時發生錯誤: {e}")
        # 如果您不確定 'features' 是否完全匹配，可以嘗試移除它，但分割邏輯保持不變
